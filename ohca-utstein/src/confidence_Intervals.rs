...
fn Calculate_confidence_interval(mean: f32, confidence: i32, standard_deviation: f32, sample_size:i32, z_value:f32) ->f32   
{
    if sample_size < 30{
  confidence_interval = mean + confidence (standard_deviation / sqrt(sample_size))
} else {
    confidence_interval = z * standard_deviation + (z * (standard_deviation / sqrt(sample_size)))
